...
class MD:
    ...
    def ltps(self) -> Dict[str, float]:
        return {str(r.symbol): float(r.ltp)
...
TABS = {1: "MARKET", 2: "PORTFOLIO", 3: "SIGNALS", 4: "CALENDAR", 5: "LOOKUP"}
...
def exec_buy(sym: str, sh: str, pr: str) -> str:
    ...
    save_port(port)
    return f"BUY  {shares}x{sym} @ {price:.1f}  cost {cost:,.0f} NPR  fees {fees:.0f}"
...
def exec_sell(sym: str, sh: str, pr: str) -> str:
    ...
    save_port(port)
    return f"SELL  {sell}x{sym} @ {price:.1f}  proceeds {price*sell:,.0f} NPR"
...
def main():
    ...
    d   = MD(args.top)
    tab = 1; lk = ""
    msg = f"Session: {d.latest}  │  ▲{d.adv} ▼{d.dec}  │  Auto-refresh {args.refresh}s"
