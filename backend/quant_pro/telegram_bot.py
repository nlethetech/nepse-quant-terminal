...
from backend.quant_pro.tms_audit import load_execution_intent, update_execution_intent
from backend.quant_pro.tms_models import ExecutionStatus, utc_now_iso
...
def _format_refresh_meta(trader: "LiveTrader") -> list[str]:
    ...
    lines: list[str] = []
    if trader.last_refresh:
        delta_secs = max(0, int((now_nst() - trader.last_refresh).total_seconds()))
        mins, _ = divmod(delta_secs, 60)
        lines.append(f"Marks: {trader.last_refresh.strftime('%b %d %H:%M')} NST")
...
def _build_owner_help_message() -> str:
    return (
        ...
        "/tms_orders — Daily and historic order book snapshot\n"
...
def _build_owner_status_message(trader: "LiveTrader") -> str:
    ...
    last_refresh = trader.last_refresh.strftime("%H:%M") if trader.last_refresh else "--"
...
def _build_tms_orders_message(trader: "LiveTrader", *, force: bool = False) -> str:
...
def _build_owner_daily_message(trader: "LiveTrader") -> str:
    ...
    snapshot_nst = daily.get("last_refresh_nst")
...
@authorized
async def cmd_tms_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    trader = _get_trader()
    if not trader:
        await update.message.reply_text("Trader not initialized.")
        return
    await asyncio.to_thread(functools.partial(trader.refresh_tms_monitor, force=True, emit_alerts=False))
    await update.message.reply_text(_build_tms_orders_message(trader, force=False), parse_mode="HTML")
...
async def live_confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(raw_result, dict):
        from backend.quant_pro.tms_models import ExecutionResult
...
OWNER_COMMANDS: Sequence[str] = (
    ...
    "tms_orders", "tms_trades", "tms_health", "orders_live", "positions_live",
...
def _build_application(
    ...
    *,
    ...
    if role == "viewer":
        ...
        app.add_handler(CommandHandler("tms_orders", cmd_tms_orders))
